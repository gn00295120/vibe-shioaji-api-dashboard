...
| 文件 | 說明 |
|------|------|
| [部署教學](docs/DEPLOYMENT.md) | Synology NAS / VPS / 本地開發部署指南 |
| [API 文件](docs/API.md) | 完整 API 端點參考 |
| [Webhook 設定](docs/WEBHOOK.md) | TradingView Webhook 整合教學 |
| [前端部署](docs/FRONTEND.md) | Dashboard 前端部署指南 |
...
- ☁️ **Multi-tenant SaaS 模式** - 支援多租戶，每個用戶獨立 Worker 容器
- 🔒 **加密憑證管理** - AES-256 加密儲存 API 金鑰
...
### Multi-tenant SaaS 模式

```
┌─────────────────┐      ┌─────────────────┐      ┌─────────────────┐
│   TradingView   │      │     Gateway     │      │    Admin API    │
│    Webhook      │ ───► │   (Port 9879)   │      │   (Port 9880)   │
└─────────────────┘      └────────┬────────┘      └────────┬────────┘
                                  │                        │
                                  ▼                        ▼
                         ┌─────────────────┐      ┌─────────────────┐
                         │      Redis      │      │ Worker Manager  │
                         │     (Queue)     │      │  (Docker API)   │
                         └────────┬────────┘      └────────┬────────┘
                                  │                        │
                    ┌─────────────┼─────────────┐          │
...
> 📖 詳細架構說明請參考 [部署教學](docs/DEPLOYMENT.md)
...
> 📖 完整 API 參考請參考 [API 文件](docs/API.md)
...
> 📖 完整設定教學請參考 [Webhook 設定指南](docs/WEBHOOK.md)
...
- [部署教學](docs/DEPLOYMENT.md) - Synology NAS / VPS / 本地開發
- [API 文件](docs/API.md) - 完整 API 端點參考
- [Webhook 設定](docs/WEBHOOK.md) - TradingView 整合教學
- [前端部署](docs/FRONTEND.md) - Dashboard 前端部署
